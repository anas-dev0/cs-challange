...
export default function Interview() {
  ...
  // Get session data from ServiceContext (stored during upload)
  const cvFilename = (interviewConfig as any).cvFilename || "uploaded-cv.pdf";
  const jobDescription = interviewConfig.description || "";
  const candidateEmail =
    (interviewConfig as any).candidateEmail || "user@example.com";
  ...
  useEffect(() => {
    const startSession = async () => {
      try {
        ...
        const resp = await fetch(`${TOKEN_SERVER_URL}/api/start-session`, {
          method: "POST",
          headers: headers,
          body: JSON.stringify({
            name: candidateName,
