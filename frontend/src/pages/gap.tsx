...
import { UploadCloud, FileText, CheckCircle, AlertTriangle, ArrowRight, Brain, BookOpen, Edit3, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function SkillsGapAnalyzer() {
  ...
  const gapSkills = report?.job_skill_profile.filter((s: GapItem) => s.gap > 0) || [];
  const matchSkills = report?.job_skill_profile.filter((s: GapItem) => s.gap <= 0) || [];
      ...
      {report && (
        <div className="mt-24 space-y-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {renderGapList("Missing Skills (Your Gaps)", <AlertTriangle className="text-yellow-400" />, gapSkills, showAllMissing, setShowAllMissing)}
            {renderGapList("Matched Skills (Your Strengths)", <CheckCircle className="text-green-400" />, matchSkills, showAllMatched, setShowAllMatched)}
          </div>
